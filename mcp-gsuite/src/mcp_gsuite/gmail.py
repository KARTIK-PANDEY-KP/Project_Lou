...
class GmailService():
    ...
    def get_draft_id_from_message_id(self, message_id: str) -> str | None:
        """
        Get the draft ID for a given message ID.
        
        Args:
            message_id (str): The message ID to look up
            
        Returns:
            str: The draft ID if found, None otherwise
        """
        try:
            # List all drafts
            ...
            for draft in drafts:
                # Get the full draft to check its message ID
                full_draft = self.service.users().drafts().get(
                    userId='me',
                    id=draft['id']
                ).execute()
                
                if full_draft.get('message', {}).get('id') == message_id:
                    return draft['id']
                    
            return None
            
        except Exception as e:
            logging.error(f"Error getting draft ID: {str(e)}")
            logging.error(traceback.format_exc())
            return None
    ...
    def delete_draft(self, message_id: str) -> bool:
        ...
        Args:
            message_id (str): The message ID of the draft to delete
            
        Returns:
            bool: True if deletion was successful, False otherwise
        """
        try:
            # First get the draft ID from the message ID
            draft_id = self.get_draft_id_from_message_id(message_id)
            if not draft_id:
                logging.error(f"No draft found for message ID: {message_id}")
                return False
                
            # Delete the draft using its ID
    ...
    def create_reply(self, original_message: dict, reply_body: str, send: bool = False, cc: list[str] | None = None) -> dict | None:
        ...
        try:
            ...
            if send:
                ...
                logging.info(f"sending reply to {to_address}")
    ...
    def send_email(self, to: str, subject: str, body: str, cc: list[str] | None = None) -> dict | None:
        ...
        Returns:
            dict: Sent message data if successful, None otherwise
        """
        try:
            # Create message body
            message = {
